...
- A public gateway that serves content from an in-page service worker cannot be redirected once the worker takes over its origin, and this cannot be fixed easily inside Companion. Example: [inbrowser.link](https://inbrowser.link), the [service-worker-gateway](https://github.com/ipfs/service-worker-gateway), registers a root-scope (`scope: '/'`) worker on first load. `declarativeNetRequest` and the observe-then-`tabs.update` fallback only act on requests that reach the network stack; a navigation answered by the worker from CacheStorage (or any worker-generated response) never reaches it, so no redirect rule matches and no `webRequest` event is observed. The state survives reloads because the worker registration and its cache persist, so a redirect to the local gateway never happens no matter how many times the user refreshes. This is a documented Chromium limitation, see the [chromium-extensions thread](https://groups.google.com/a/chromium.org/g/chromium-extensions/c/K2DtYbwtCAA). MV2 Firefox is not affected because its blocking `webRequest` redirects the navigation itself before the worker handles it, which the declarative model cannot do. Companion reaches this state because the `?x-ipfs-companion-no-redirect` opt-out (or a lost first-load race) lets the worker register in the first place; once it controls the origin, Chromium will not hand the navigation back at the network layer. To recover, Companion watches `webNavigation.onCommitted` (`recoverRedirectMiss` in `ipfs-request.js`): when a top-level navigation commits to serviceable public gateway content and a local gateway is available, it sends the tab to the local gateway with `tabs.update`. This still honors the opt-out (a URL carrying the token is left alone) and skips Back/Forward navigations so it does not trap the Back button. A user who wants to stay on the public worker can also clear it with the gateway's own `?ipfs-sw-unregister` escape hatch.
