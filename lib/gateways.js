...
function getPublicGatewayHostsRegex() {
  ...
  for (var i=0; i < hosts.length; i++) {
    ...
    hostsRegex += hosts[i].replace(/[^\w\s]/g, '\\$&');
  }
...
function reloadCachedProperties(changedProperty) { // jshint unused:false
  ...
  for (var i=0;i<menuContexts.length;i++) {
    ...
    if (CUSTOM_GATEWAY_URI) {
      context.remove(cm.URLContext(CUSTOM_GATEWAY_URI.spec + proto.ipfsScheme + '*'));
      context.remove(cm.URLContext(CUSTOM_GATEWAY_URI.spec + proto.ipnsScheme + '*'));
    }
    CUSTOM_GATEWAY_URI = ioservice.newURI('http://' + prefs.customGatewayHost + ':' + prefs.customGatewayPort, null, null);
    context.add(cm.URLContext(CUSTOM_GATEWAY_URI.spec + proto.ipfsScheme + '*'));
    context.add(cm.URLContext(CUSTOM_GATEWAY_URI.spec + proto.ipnsScheme + '*'));
  };
}
