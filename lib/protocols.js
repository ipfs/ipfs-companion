...
exports.rewrite = function (url) {
  ...
  // relative path
  // TODO: replace split() below with Path-validation from next version of  is-ipfs
  const ipfsPrefix = '/ipfs/'
  const ipfsHash = url.split('#')[0].split('/')[2]
  if (url.startsWith(ipfsPrefix) && isIPFS.multihash(ipfsHash)) {
