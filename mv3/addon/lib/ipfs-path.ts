...
export default function createIpfsPathValidator (getState, client, dnslinkResolver) {
  const ipfsPathValidator = {
    ...
    resolveToCid: pMemoize(async function (urlOrPath) {
      ...
      const result = await client.resolve(rawPath, { recursive: true, dhtt: '5s', dhtrc: 1 })
