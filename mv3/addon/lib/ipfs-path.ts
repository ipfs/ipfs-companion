...
export function ipfsContentPath(urlOrPath, opts) {
  ...
  if (isIPFS.subdomain(urlOrPath)) {
    ...
    url = new URL(
      `https://localhost/${ns}/${id}${url.pathname}${url.search}${url.hash}`
    );
  }
...
export const safeHostname = (url) => {
  ...
  try {
    ...
    console.error(
      `[ipfs-companion] safeHostname(url) error for url='${url}'`,
      e
    );
  }
  return null;
...
export default function createIpfsPathValidator(
  getState,
  client,
  dnslinkResolver
) {
  const ipfsPathValidator = {
    ...
    isIpfsPageActionsContext(url) {
      if (!url || isNewTabURL(url)) return false;
      const { apiURLString } = getState();
      const { hostname } = new URL(url);
      const cachedHostname = dnslinkResolver.hasCachedDnslink(hostname);
      return Boolean(
        url &&
          !url.startsWith(apiURLString) &&
          (!!ipfsContentPath(url) || cachedHostname)
      );
    },
    ...
    isRedirectPageActionsContext(url) {
      ...
      return (
        localGwAvailable && // show only when redirect is possible
    ...
    resolveToPublicUrl(urlOrPath) {
      ...
      if (input.startsWith("ipns://")) {
        ...
        const dnslink = dnslinkResolver.readAndCacheDnslink(
          dnslinkUrl.hostname
        );
        if (dnslink) {
      ...
      if (isIPFS.subdomain(input)) {
        // Switch Origin to preferred public subdomain gateway (default: dweb.link)
        ...
        // and is usually not supported due to limitation of TLS wildcard certs.
    ...
    resolveToLocalUrl(urlOrPath) {
      ...
      if (isIPFS.path(ipfsPath))
        return pathAtHttpGateway(ipfsPath, gwURLString);
      return null;
    },

    // Resolve URL or path to HTTP URL with CID:
    // - IPFS paths are attached to HTTP Gateway root
    // - URL of DNSLinked websites are resolved to CIDs
    // The purpose of this resolver is to always return a meaningful, publicly
    ...
    resolveToImmutableIpfsPath: pMemoize(
      async function (urlOrPath) {
        ...
        if (isIPFS.ipnsPath(path)) {
          ...
          const result = await client.resolve(ipnsRoot, { recursive: true });
    ...
    resolveToCid: pMemoize(
      async function (urlOrPath) {
        ...
        const result = await client.resolve(rawPath, {
          recursive: true,
          dhtt: "5s",
          dhtrc: 1,
        });

        const directCid = isIPFS.ipfsPath(result)
          ? result.split("/")[2]
          : result;
        return directCid;
      },
